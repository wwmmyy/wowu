...
public class EaseShowBigImageActivity extends EaseBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (uri != null && new File(uri.getPath()).exists()) {
            ...
            maps.put("Authorization", "Bearer " + OkHttpUtils.token);
            EMLog.d(TAG, "标题头：：：：：" + OkHttpUtils.token);

            if(loadType.equals("wuho")){
                downloadIamge2(remotepath);
            }else{
    ...
    private void downloadIamge2(String remotepath) {
        ...
        final String tempPath =localFilePath;// temp.getParent() + "/temp_" + temp.getName();

        OkHttpUtils//
                .get()//
                .url(remotepath)//
                .build()//
                .execute(new FileCallBack(Environment.getExternalStorageDirectory().getAbsolutePath()+getFilePath(localFilePath),  getFileName(localFilePath))//
                {
                    @Override
                    public void inProgress(float progress)
                    {
                        final String str2 = getResources().getString(R.string.Download_the_pictures_new);
                        pd.setMessage(str2 + (int)progress*100 + "%");
                    }

                    @Override
                    public void onError(Request request, Exception e)
                    {
                        EMLog.e(TAG, "offline file transfer error:" + e.getMessage());
...
//                        pd.setProgress((int)progress*100);
                    ...
                    @Override
                    public void onResponse(final Bitmap bitmap0) {
                        Log.d(TAG, "下载成功：：：:" + bitmap0);
                            ...
                            public void run() {
                              //  new File(tempPath).renameTo(new File(localFilePath));
                                ...
                                if (bitmap0 == null) {
                                    ...
                                    EaseImageCache.getInstance().put(localFilePath, bitmap0);
                                    isDownloaded = true;
                                }
                                if (EaseShowBigImageActivity.this.isFinishing() || EaseShowBigImageActivity.this.isDestroyed()) {
                                    return;
